...
std::vector<float> load_spectrum_data(const std::string& file_path) {
    ...
    // Skip the header row of CSV file
...
std::vector<int> post_process_peaks(const std::vector<float>& predictions, int min_distance, int max_peaks = 10, float min_threshold = 0.03) {
...
// Wave peak centroid calculation function
std::vector<std::pair<int, float>> calculate_peak_centroids(const std::vector<float>& predictions, const std::vector<float>& intensity_data, float threshold = 0.2, int min_distance = 20) {
    // Step 1: Find all points with probability greater than threshold
    ...
    std::cout << "Found " << peak_candidates.size() << " candidate points above threshold" << std::endl;
    
    // Step 2: Group nearby points into peak clusters
    std::vector<std::vector<int>> peak_clusters;
    if (!peak_candidates.empty()) {
        std::vector<int> current_cluster = {peak_candidates[0]};
        
        for (size_t i = 1; i < peak_candidates.size(); ++i) {
            if (peak_candidates[i] - peak_candidates[i-1] <= min_distance) {
                // If close to previous point, add to current cluster
                current_cluster.push_back(peak_candidates[i]);
            } else {
                // Otherwise, save current cluster and start new one
                if (!current_cluster.empty()) {
                    peak_clusters.push_back(current_cluster);
                }
                current_cluster = {peak_candidates[i]};
            }
        }
    ...
    std::cout << "Formed " << peak_clusters.size() << " peak clusters" << std::endl;
    
    // Step 3: Calculate centroid for each cluster
    std::vector<std::pair<int, float>> centroids;
    for (size_t cluster_idx = 0; cluster_idx < peak_clusters.size(); ++cluster_idx) {
        const auto& cluster = peak_clusters[cluster_idx];
        if (cluster.empty()) continue;
        
        std::cout << "\nPeak Cluster #" << cluster_idx + 1 << " contains " << cluster.size() << " points:" << std::endl;
        std::cout << "Cluster point indices: ";
        for (int idx : cluster) {
            std::cout << idx << " ";
        }
        std::cout << std::endl;
        
        // Find the peak point (maximum intensity) in the cluster
        int peak_idx = cluster[0];
        float peak_intensity = intensity_data[cluster[0]];
        
        for (size_t i = 1; i < cluster.size(); ++i) {
            if (intensity_data[cluster[i]] > peak_intensity) {
                peak_intensity = intensity_data[cluster[i]];
                peak_idx = cluster[i];
            }
        }
        
        std::cout << "Peak point index: " << peak_idx << ", intensity value: " << peak_intensity << std::endl;
        
        // Define threshold for boundary detection (e.g., 50% of peak intensity)
        float boundary_threshold = peak_intensity * 0.5f;
        
        // Find left boundary by intensity
        int left_bound = peak_idx;
        while (left_bound > 0 && intensity_data[left_bound] > boundary_threshold) {
            left_bound--;
        }
        
        // Find right boundary by intensity
        int right_bound = peak_idx;
        while (right_bound < static_cast<int>(intensity_data.size()) - 1 && 
               intensity_data[right_bound] > boundary_threshold) {
            right_bound++;
        }
        
        std::cout << "Intensity-based boundaries: left=" << left_bound << ", right=" << right_bound << std::endl;
        
        // Determine centroid calculation area
        int start_idx = left_bound;
        int end_idx = right_bound;
        
        std::cout << "Centroid calculation range: from index " << start_idx << " to " << end_idx << std::endl;
        std::cout << "Data points in calculation range:" << std::endl;
        std::cout << "Index\tWavelength(nm)\tIntensity" << std::endl;
        
        // Calculate peak position using centroid method (using actual wavelength instead of index)
        float weighted_sum = 0.0f;
        float sum_weights = 0.0f;
        
        for (int i = start_idx; i <= end_idx; ++i) {
            float wavelength = map_index_to_wavelength(i);
            weighted_sum += wavelength * intensity_data[i];
            sum_weights += intensity_data[i];
            
            std::cout << i << "\t" << wavelength << "\t" << intensity_data[i] << std::endl;
        }
        
        // Calculate centroid (in wavelength domain)
        float centroid_wavelength = (sum_weights > 0) ? weighted_sum / sum_weights : map_index_to_wavelength((start_idx + end_idx) / 2);
        
        // Map wavelength centroid back to nearest index
        int centroid_idx = static_cast<int>((centroid_wavelength - 1525.0f) / (1565.0f - 1525.0f) * (predictions.size() - 1) + 0.5f);
        centroid_idx = std::max(0, std::min(static_cast<int>(predictions.size()) - 1, centroid_idx));
        
        std::cout << "Result - Centroid wavelength: " << centroid_wavelength << " nm, corresponding index: " << centroid_idx << std::endl;
        
        centroids.push_back({centroid_idx, centroid_wavelength});
    }
    
    return centroids;
}

// Model inference and post-processing function
bool run_model(const std::string& model, const std::vector<float>& data, const int& repeat)
{
    // 1. init engine
    ...
    //     uint8_t* float_ptr = input_data.data() + i * sizeof(float);
    //     std::reverse(float_ptr, float_ptr + sizeof(float));
    // }
    // Print parsed data
    ...
    fprintf(stdout, "--------------------------------------\n");
    // if (input_data.size() * sizeof(float) != io_info->pInputs[0].nSize)
    // {
    //     fprintf(stderr, "The input data size is not matched with tensor {name: %s, size: %d}.\n", io_info->pInputs[0].pName, io_info->pInputs[0].nSize);
    //     return -1;
    // }
    // std::memcpy(io_data.pInputs[0].pVirAddr, input_data.data(), input_data.size() * sizeof(float));
    
    // io_data.pInputs[0].nSize = input_data.size() * sizeof(float);
    // SAMPLE_AX_ENGINE_DEAL_HANDLE_IO
    // fprintf(stdout, "Engine push input is done. \n");
    // fprintf(stdout, "--------------------------------------\n");
    ...
    // std::vector<float> predictions(class_num);
    // for (uint32_t id = 0; id < class_num; id++)
    // {
    //     predictions[id] = ptr[id];
    // }
    ...
    auto peak_positions = post_process_peaks(predictions, 20, 16 , 0.03);
    std::cout << "Predicted Peaks: ";
    ...
    // Calculate peak centroids and measure time consumption
    timer centroid_tick;
    auto peak_centroids = calculate_peak_centroids(predictions, data, 0.2, 20);
    float centroid_time = centroid_tick.cost();
    
    // Output peak centroid results and time consumption
    std::cout << "Peak Centroids (position, exact_position): ";
    for (const auto& centroid : peak_centroids) {
        fprintf(stdout, "(%d, %.4f) ", centroid.first, centroid.second);
    }
    std::cout << std::endl;
    fprintf(stdout, "Centroid calculation time: %.2f ms\n", centroid_time);
...
int main(int argc, char* argv[])
{
    ...
    {
        run_model(model_file, spectrum_data_filtered, repeat);
